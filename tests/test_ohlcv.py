...
class OHLCVTestCase(unittest.TestCase):
    ...
    def __init__(self, methodName: str = "runTest") -> None:
        ...
        self.df = pd.DataFrame(arr, index=index, columns=columns).astype('float64')
    ...
    def test_ohlcv_nan(self):
        ...
        df = self.df.copy()
        nan_mask = np.random.random(df.shape) > .8
        ...
        np.testing.assert_array_equal(my_result.values[~my_nan_mask], pd_result.values[~pd_nan_mask])
